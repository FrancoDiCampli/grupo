    ...
    <div>
        <v-tooltip left>
            <template v-slot:activator="{ on }">
                <v-btn color="secondary" dark fab fixed right bottom large v-on="on" to="/roles">
...
export default {
    ...
    mounted() {
        if (this.$store.state.roles.form.role) {
            this.setScopes();
        } else {
    ...
    methods: {
        setScopes: async function() {
            let permisos = await this.$store.dispatch("roles/permissions");
            let scopes = this.$store.state.roles.form.permission.split(" ");
            let selected = [];

            for (let i = 0; i < scopes.length; i++) {
                let find = permisos.find(
                    permiso => permiso.permission === scopes[i]
                );

                if (find) {
                    selected.push(scopes[i]);
                }
            }

            this.$store.state.roles.form.scope = selected;
        },
